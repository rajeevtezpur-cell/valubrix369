...
// Phase 1: 6-step form (Location → Property Type → Details → Budget → Builder/Project → Summary)
// Phase 2: Listings-first results + secondary AI insights toggle
import {
  Brain,
  ChevronDown,
  ChevronRight,
...
function computeBadgeFlags(listing: MockListing, mode: "buy" | "rent") {
  ...
  const isHighLiquidity = dealScore > 70 || recommendation === "Strong Buy";
  const isDistressDeal =
  ...
  const isHotMarket = recommendation === "Strong Buy" || dealScore > 80;
  return { isHighLiquidity, isDistressDeal, isHighYield, isHotMarket };
}
...
export default function BuyRentSearchPage({ mode }: BuyRentSearchPageProps) {
  ...
  // ── Form engine (used for future extensions — data managed locally for full step control)
  usePropertyForm(mode, 1);

  // ── Local step state (we manage steps in this page for full control)
  const [step, setStep] = useState(1);
  const [stepErrors, setStepErrors] = useState<Record<string, string>>({});

  // ── Map center (updated when location is selected)
  const [mapCenter, setMapCenter] = useState<[number, number]>([
    12.9716, 77.5946,
  ]);

  // ── "Pin on Map" full-screen modal state
  ...
  // BUG 4 FIX: When city changes, reset locality, map center, coordinates
  useEffect(() => {
    setSelectedLocation(null);
    // Reset map to city center at zoom 11
    ...
    const newCenter = cityCenters[city] ?? [12.9716, 77.5946];
  ...
  // ── Results: AI insights toggle
  const [showInsights, setShowInsights] = useState(false);

  // ── Saved/heart state for listings (reserved for future use)
  ...
  const uniqueBuilders = useMemo(() => {
    const s = new Set(BANGALORE_PROJECTS.map((p) => p.builder));
  ...
  // ── Filter listings based on form data
  const filtered = useMemo(() => {
    return allListings.filter((l: MockListing) => {
      if (selectedLocation) {
  ...
  // ── OSRM source coords — always use property/map center coords ───────────────
  const srcLat = mapCenter[0];
  const srcLng = mapCenter[1];

  // ── OSRM-based nearby POIs (replaces haversine)  ─────────────────────────────
  ...
  useEffect(() => {
    ...
    console.log("[OSRM] BuyRent Source:", srcLat, srcLng);
    ...
    Promise.all([
      ...
      if (metros[0]) {
        console.log(
          `[OSRM] Result sample: ${metros[0].name} ${metros[0].osrmKm}km ${metros[0].osrmDurationMins}mins`,
        );
      }
    });
  ...
  // ── Static nearby POI panel for map-pin display (smart pins - non-driving use)
  const nearbyPins = useMemo(() => {
    const [lat, lng] = mapCenter;
    const pins = getSmartPins(lat, lng, 12);
    type PinCategory = "tech_park" | "metro" | "railway" | "bus_stop";
    ...
    const result: Record<PinCategory, SmartPin[]> = {
      tech_park: [],
      metro: [],
      railway: [],
      bus_stop: [],
    ...
    for (const pin of pins) {
      if (categories.includes(pin.type as PinCategory)) {
        result[pin.type as PinCategory].push(pin);
      }
    }
  ...
  function validateAndNext() {
    ...
    if (step === 1) {
      // BUG 7 FIX: Require BOTH city AND (locality OR pin placed)
  ...
  function handleSubmit() {
    // Show analyzing overlay, data loads in parallel
  ...
  if (phase === "form") {
    return (
      <>
          ...
          >
            ...
            >
              ...
              <button
                ...
                data-ocid="buyrent.map_modal.close"
              >
                <X size={16} />
              </button>
            </div>
        ...
        {/* Root container — fixed, full viewport */}
        <div
          style={{
            position: "fixed",
            inset: 0,
        ...
        {/* Mode toggle — absolute centered below navbar */}
        <div
          style={{
            position: "fixed",
            top: 72,
        ...
        {/* Floating form card — absolute, top-left, does NOT push map */}
        <div
          ref={formRef}
          style={{
            position: "fixed",
        ...
        >
          ...
          >
            ...
            {step === 1 && (
              <div className="space-y-5">
                    ...
                    {CITIES.map((c) => (
                      <button
                        ...
                        style={{
                          ...
                          color:
                            city === c ? "#D8B56A" : "rgba(255,255,255,0.65)",
                        }}
                      >
                        {c}
                      </button>
                ...
                <div>
                  ...
                  >
                    <SmartLocationSearch
                      ...
                      onSelect={(loc) => {
                        ...
                        // Sync map center — look up lat/lng for the selected locality
                  ...
                  >
                    <MapPin size={14} />
                    Pin on Map — Drop pin to select exact location
                  </button>
                </div>
                ...
                {selectedLocation && (
                  ...
                  >
                    <MapPin size={14} />
                    View Area Intelligence for {selectedLocation.name}
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setSelectedBudget(null); // Reset budget when type changes
                        }}
            ...
            {/* ── Step 3: Property Details (dynamic, all optional) ─ */}
            {step === 3 && (
              <div className="space-y-5">
                <div>
                  <h2
                ...
                {/* BHK — only for apartment/builder_floor/studio */}
                ...
                {/* Floor preference — only for apartment/builder_floor */}
                ...
                {/* Area input — optional for buy/rent */}
  ...
  return (
    <div ref={resultsRef} style={{ background: "#071A2F", minHeight: "100vh" }}>
      <AnalyzingOverlay
        isVisible={showOverlay}
        module={mode}
        dataReady={overlayDataReady}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-24">
          ...
          {(nearbyPins.tech_park.length > 0 ||
            ...
            >
              ...
              {/* OSRM-based distances when available, fallback to smart pins */}
              {osrmNearbyMetros.length > 0 ||
              osrmNearbyTechParks.length > 0 ||
              ...
              osrmNearbyBusStops.length > 0 ? (
                ...
                /* Fallback to smart pins (no OSRM data — default center or loading) */
                      ...
                      <div key={cat.key} className="space-y-1.5">
                        ...
                        {nearbyPins[cat.key].slice(0, 4).map((pin) => (
                          ...
                          >
                            ...
                            >
                              Select location to get driving distances
                            </span>
                          </div>
                        ))}
                      </div>
          ...
          {/* ── AI Price Intelligence (secondary, collapsible) ─ */}
          <div
          ...
          >
            <button
              ...
              style={{ background: "rgba(216,181,106,0.07)" }}
            >
...
// ─── Type alias (needed since GlobalMapComponent import types not re-exported) ─
